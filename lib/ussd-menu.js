...
class UssdMenu extends EventEmitter {
    ...
    constructor(opts = {}) {
        super();
        const validProviders = ['hubtel', 'africasTalking', 'emergent', 'cross-switch', 'nalo', 'arkesel', 'southpawsl', 'cross-switch'];
    ...
    con(text) {
        ...
        } else if (this.provider === 'southpawsl' || this.provider === 'cross-switch') {
            this.result = {
                message: text,
                state: "CONTINUE",
                menuId: this.args.menuId,
                ussdString: this.args.ussdString,
                option: "",
                ussdParameter: []
            };
        } else {
    ...
    mapArgs(args) {
        if (this.provider === 'hubtel') {
            ...
            this.args = {
                ...
                text: args.MSGTYPE === true ? this.parseNolaInitiationText(args) : args.USERDATA,
            };
        } else if (this.provider === 'arkesel') {
            // args.Type = args.MSGTYPE === true ? "Initiation" : "Response";
            // args.Text = '',
            ...
            this.args = {
                ...
                serviceCode: '',
                operator: args.network,
                text: args.newSession === true ? this.parseArkeselInitiationText(args) : args.userData,
            };
        } else if (this.provider === 'southpawsl' || this.provider === 'cross-switch') {
            // args.Type = args.MSGTYPE === true ? "Initiation" : "Response";
            // args.Text = '',
            ...
            this.args = {
                ...
                operator: args.network,
                text: args.ussdString === args.inputOption ? this.parseInitiationText(args) : args.inputOption,
            };
        } else {
            this.args = args;
        }
    ...
    getRoute(args) {
        if (this.provider === 'hubtel') {
            ...
            if (this.session === null) {
                ...
                // Parse initial message for a route
                const route = this.parseNolaInitiationText(args);
                return this.session.set('route', route).then(() => route);
            } else {
                return this.session.get('route').then(pastRoute => {
                    const route = pastRoute ? `${pastRoute}*${this.args.text}` : this.args.text;
